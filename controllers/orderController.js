...
const placeOrder = async (req, res) => {
    try {
        ...
        items.forEach(({ _id, size }) => {
            if (updatedCart[_id] && updatedCart[_id][size]) {
                delete updatedCart[_id][size]; // Remove item-size combination from cart
                if (Object.keys(updatedCart[_id]).length === 0) {
                    delete updatedCart[_id]; // Remove product if no sizes left
                }
            }
        });
